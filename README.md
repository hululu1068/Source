<p align="left">
  <img src="include/logo.png" alt="OpenWrt logo" width="320" />
</p>
...
## 编译 (推荐Ubuntu22.04)
  - 官方推荐依赖（常用精简版）：
    ...
    # ARM64机器将 gcc-multilib 替换为 gcc-arm-linux-gnueabihf
    ```
  - ImmortalWrt (支持第三方包)
    ```bash
    sudo apt update -y
  ...
  - 编译命令 (基础)
    ...
    make menuconfig
    make -j$(nproc) V=s                 # 推荐用 nproc 线程，首次可用 -j1
    make -j1 V=s | tee ../compile.log   # 将日志保存到 ../compile.log
    # 查看错误
    grep -n "ERROR:" ../compile.log
    grep -n "failed" ../compile.log
    ```
...
## 编译常见问题
  ...
  - 移除官方 openwrt feeds 自带的核心包 (通常更新不及时)
    ```bash
    ./scripts/feeds update -a -f && ./scripts/feeds install -a -f       # install feeds后
    rm -rf feeds/packages/net/{xray-core,v2ray-core,v2ray-geodata,sing-box}
    ```

  - 替换 golang 版本 (解决v2ray/xray编译报错)
    ```bash
...
## ❇️ 常用插件对照
  - 以`feeds`为准.
    ```bash
    LuCI-app-*：         注：应用后面标记 “ * ” 为最近新添加；标记“ ! ”与其他插件依赖或冲突。
    -----------------------------------------------------------------------------------------
